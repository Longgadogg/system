...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset ($_POST['submit'])) {
    ...
    $stmt = $conn->prepare("INSERT INTO coursedb (professorname, code, title, pic) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $professorname, $code, $title, $targetFile);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset ($_POST['search'])) {
    ...
    $searchQuery = " WHERE professorname LIKE '%$search%' OR code LIKE '%$search%' OR title LIKE '%$search%'";
}

// Construct the SQL query to fetch data based on the search query
$sql = "SELECT * FROM coursedb" . $searchQuery . " LIMIT 5";
            ...
            <div class='modal-dialog modal-lg modal-dialog-centered'>
                <div class='modal-content'>
                    ...
                    <div class='modal-body'>
                        ...
                        <div class='mb-3'>
                            <strong>Course Code:</strong> " . $row["code"] . "
                        </div>
                        <div class='mb-3'>
                            <strong>Course Title:</strong> " . $row["title"] . "
                        </div>
                        <div class='text-center'>
                            <button type='button' class='btn btn-danger'>
                                <a href='deletecourse.php?deleteid=".$row["id"]."' class='text-white text-decoration-none'>Delete</a>
                            </button>
